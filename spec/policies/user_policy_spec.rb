...
describe UserPolicy do
  ...
  let(:sp_trigger_password_reset_email) { create(:system_permission, name: "#{klass} Trigger Password", resource: klass, operation: 'trigger_password_reset_email') }

  before do
    system_role.system_permissions << [sp_trigger_password_reset_email]
  end
  ...
  describe '#trigger_password_reset_email?' do
    it 'allows access if user has destroy permission' do
      expect(policy.trigger_password_reset_email?).to be_truthy
    end

    it 'denies access if user does not have destroy permission' do
      system_role.system_permissions.delete(sp_trigger_password_reset_email)
      expect(policy.trigger_password_reset_email?).to be_falsey
    end
  end
end
